...
def find_peaks(Data, WG_Width, Valley_Offset, FindPeak_thresh=0.5, FindPeak_mindist=1.0, \
    ...
    with plt.style.context((['default', 'fitplotstyle'])):
        ...
        Idx, Idx_xdata, FWHM, FWHM_STD, Peak_to_Valley, Peak_to_Valley_STD, Delta_X, Delta_X_STD = [], [], [], [], [], [], [], []
        if (np.amin(Data) != np.amax(Data)):
            try:
                Idx = peakutils.indexes(Data, thres=FindPeak_thresh, \
                min_dist=FindPeak_mindist/WG_Width*len(Data))
            except:
                pass
        if (len(Idx) > 0):
    ...
    ##                               sigma=(0.9 - 0.8*np.ceil((ydata-FindPeak_thresh)/np.amax(ydata))), bounds=bnds, method='trf')
            ...
            popt, pcov = curve_fit(eval('multi_gauss'), xdata, ydata, p0=p0, bounds=bnds, method='trf')
    ...
    #        FWHM = 2.35482*popt[2::3]
            ...
            return Idx, Idx_xdata, FWHM, FWHM_STD, Peak_to_Valley, Peak_to_Valley_STD, Delta_X, Delta_X_STD
        if (len(Idx) > 1):
